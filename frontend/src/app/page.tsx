            ...
            {traces.length === 0 ? (
              ...
              <div className="divide-y divide-slate-800/60 max-h-[600px] overflow-y-auto">
                {traces.map((trace: any) => (
                  <div key={trace.trace_id} className="p-4 hover:bg-slate-800/30 transition duration-200 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-bold ${trace.status === "success"
                            ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30"
                            : "bg-red-500/20 text-red-500 border border-red-500/30 animate-pulse"
                            }`}
                        >
                          {trace.status === "success" ? "EXECUTED" : "KILL SWITCH EVALUATION"}
                        </span>
